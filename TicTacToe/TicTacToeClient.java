...
public class TicTacToeClient extends JApplet implements Runnable {
    ...
    private DataOutputStream outputBoth;
    ...
    public void init() {
        ...
        panel2.add(rstButton, BorderLayout.NORTH);
        panel2.add(endButton, BorderLayout.NORTH);

        endButton.addActionListener(new ActionListener(){
        @Override
        public void actionPerformed(ActionEvent e) {
            logger.info("Kill connection");
            System.exit(0);
        }
        ...
        rstButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if(gameOver==true){
                    ...
                    board = new Square[ 3 ][ 3 ];  //could have done one array of [9]
                    for ( int row = 0; row < board.length; row++ ) {
                        for ( int column = 0; column < board[ row ].length; column++ ) {
                            // create Square
                            board[ row ][ column ] = new Square( ' ', row * 3 + column );
                    ...
                    gameOver = false;
                    logger.info("Client Cleared and reset");
                    run();
                }
                else{logger.info("Game not over !!!!");}
            }
        });
    ...
    public void start() {
        try {
            FileHandler handler = new FileHandler("TicTacToeClient-log.%u.%g.txt",true);
            logger.log(Level.SEVERE, "Logging!!!");//will only print to console
            logger.addHandler(handler);
            SimpleFormatter formatter = new SimpleFormatter();
            handler.setFormatter(formatter);

            logger.info("Logging clients ");//print to log txt file
    ...
    private void processMessage(String message) {
        ...
        logger.info("String recieved "+ message);
        logger.info("message split "+newString[0] +newString[1] );
        ...
        else if((newString[0].equals("GAME"))&&(newString[1].equals("OVER"))){
            displayMessage(message + "\n");

            logger.info("Message game over recieved ");//print to log txt file
            gameOver = true;
        }
